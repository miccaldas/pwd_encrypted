        ...
        with open("sitio_choice.txt", "r") as f:
            ...
            if records:
                with open("site_repeat.bash", "w") as f:
                    f.write("#!/usr/bin/env bash\n\n")
                    f.write("tput clear\n\n")
                    f.write(f"tput cup {cnf['init_height']} {cnf['init_width']}\n")
                    f.write(f"tput setaf {cnf['title_color']}\n")
                    f.write("tput bold\n")
                    ...
                    f.write(
                        f"tput cup {cnf['separator_height']} {cnf['init_width']}\n"
                    )
                    f.write(f"echo '{cnf['separator']}'\n")
    ...
    if "author_choice.txt" in lst_files:
        ...
        with open("second_input.bash", "w") as f:
            ...
            if author == "n":
                ...
                f.write(
                    f"tput cup {cnf['separator_height'] + 4} {cnf['init_width']}\n"
                )
            ...
            if "lgth_choice.txt" in lst_files:
                with open("lgth_choice.txt", "r") as r:
                    ...
                    try:
                        ...
                        if err_msg:
                            return 'Too many queries in function.', err_msg
...
def add_feedback_window(self):
    ...
    for n, t in zip(names, teste):
        ...
        # defined before.  ' | ' as line separator, and does the same for the
